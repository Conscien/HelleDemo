...
bool H265CpuEncoder::Init(int width, int height) {
  ...
  int nFrameRate = 25;
  //        int64_t nBitRate = (int64_t)((width_ * height_ * 3 / 2) * 0.8); //
  //        计算码率
  ...
  //  Y,U,V内存连续，1字节对齐
  av_image_alloc(avFrame_->data, avFrame_->linesize, codec_ctx_->width,
                 codec_ctx_->height, codec_ctx_->pix_fmt, 1);

  // 临时
  frame_tmp_ = av_frame_alloc();
  av_image_alloc(frame_tmp_->data, frame_tmp_->linesize, width_, height_,
                 codec_ctx_->pix_fmt, 1);
  img_convert_ctx_ = sws_getContext(
      width_, height_, codec_ctx_->pix_fmt, avFrame_->width, avFrame_->height,
      (AVPixelFormat)avFrame_->format, SWS_BILINEAR, nullptr, nullptr, nullptr);
...
int H265CpuEncoder::Encode(uint8_t *data, size_t data_size, uint8_t *dst_data,
                           int &out_len) {
  int ret = -1;
  out_len = 0;
  ...
  if (data != nullptr && data_size != 0) {
    ...
    ret = sws_scale(img_convert_ctx_, frame_tmp_->data, frame_tmp_->linesize, 0,
                    height_, avFrame_->data, avFrame_->linesize);
  }
  ...
  //        ret = avcodec_encode_video2(codec_ctx_, &pkt, avFrame_,
  //        &got_output); if (ret < 0) {
  //            std::cout << "Error encoding frame ret: " << ret << std::endl;
  //            return -1;
  //        }
  //        if (got_output) {
  //            std::cout << "Succeed to encode frame id : " << frame_id-1 << "
  //            get size: " << pkt.size << std::endl; memcpy(dst_data, pkt.data,
  //            pkt.size); out_len = pkt.size; av_packet_unref(&pkt); return 0;
  //        }
  ...
  //            }
  //            if (got_output) {
  //                printf("Flush Encoder: Succeed to encode 1
  //                frame!\tsize:%5d\n",pkt.size); fwrite(pkt.data, 1, pkt.size,
  //                fp_out); av_free_packet(&pkt);
  //            }
  //        }
  ...
  while (ret >= 0) {
    ...
    if (ret == AVERROR(EAGAIN)) {
      std::cout << "need more input data now frame id: " << frame_id - 1
                << " ret: " << ret << std::endl;
      break;
    } else if (ret == AVERROR_EOF) {
      std::cout << "input data end now frame id: " << frame_id - 1
                << " ret: " << ret << std::endl;
      break;
    } else if (ret < 0) {
      std::cout << "avcodec_receive_packet failed frameId: " << frame_id - 1
                << std::endl;
      return -1;
    }
    ...
    addLen += pkt.size;
    std::cout << "encoder packet " << pkt.pts << +" success len: " << pkt.size
              << std::endl;
    av_packet_unref(&pkt);
  }

  out_len = addLen;
  std::cout << "out encode data id： " << frame_id - 1
            << +" out len: " << out_len << std::endl;
  return 0;
}
} // namespace HelloDemo
