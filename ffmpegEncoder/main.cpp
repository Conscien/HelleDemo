...
static void encode(int inputFrameId, AVCodecContext *enc_ctx,
                   const AVFrame *frame, AVPacket *pkt, FILE *outfile) {
  int ret;
...
int main() {
    ...
    HelloDemo::H265CpuEncoder encoder;
...
int main() {
  ...
  //    avcodec_find_encoder_by_name("hevc_videotoolbox"); mac硬件加速
  ...
  //  Y,U,V内存连续，1字节对齐
  av_image_alloc(frame->data, frame->linesize, encoder_ctx->width,
                 encoder_ctx->height, encoder_ctx->pix_fmt, 1);
#endif
  ...
  if (needSws) {
    ...
    av_image_alloc(frame_tmp->data, frame_tmp->linesize, in_width, in_height,
                   in_pix_fmt, 1);

    img_convert_ctx = sws_getContext(
        in_width, in_height, in_pix_fmt, frame->width, frame->height,
        (AVPixelFormat)frame->format, SWS_BILINEAR, nullptr, nullptr, nullptr);
  }
  ...
  while (!feof(in_file)) {
    ...
    if (needSws) {
      if (fread(frame_tmp->data[0], in_width * in_height * 3 / 2, 1, in_file) !=
          1)
        break;
    } else {
      if (fread(frame->data[0], in_width * in_height * 3 / 2, 1, in_file) != 1)
    ...
    if (needSws) {
      ret = sws_scale(img_convert_ctx, frame_tmp->data, frame_tmp->linesize, 0,
